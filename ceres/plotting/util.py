...
def get_filenames(directory: Path) -> List[Path]:
    ...
    try:
        for child in directory.iterdir():
            if not child.is_dir():
                # If it is a file ending in .plot, removes K31 plots, MMX plots and MacOS ._ files
                if child.suffix == ".plot" and not child.name.startswith("plot-k31") and not child.name.startswith("plot-mmx") and not child.name.startswith("._"):
