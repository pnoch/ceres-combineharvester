...
def class_for_type(type: NodeType) -> Any:
    if type is NodeType.FULL_NODE:
        ...
        from ceres.wallet.wallet_node_api import WalletNodeAPI

        return WalletNodeAPI
    elif type is NodeType.INTRODUCER:
        from ceres.introducer.introducer_api import IntroducerAPI

        return IntroducerAPI
    elif type is NodeType.TIMELORD:
        from ceres.timelord.timelord_api import TimelordAPI

        return TimelordAPI
    elif type is NodeType.FARMER:
        from ceres.farmer.farmer_api import FarmerAPI
