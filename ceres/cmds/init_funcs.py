...
from ceres.ssl.create_ssl import (
    ensure_ssl_dirs,
...
from ceres.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'ceres keys generate'")
        return None
...
#             print("** Please run `ceres init` to migrate or create new config files **")
#     else:
