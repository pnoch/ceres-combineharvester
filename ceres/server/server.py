...
from ceres.server.introducer_peers import IntroducerPeers
from ceres.server.outbound_message import Message, NodeType
from ceres.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from ceres.util.network import is_localhost, is_in_network
from ceres.util.ssl import verify_ssl_certs_and_keys
...
def ssl_context_for_root(
    ca_cert_file: str, *, check_permissions: bool = False, log: Optional[logging.Logger] = None
) -> Optional[ssl.SSLContext]:
    if check_permissions:
...
class ChiaServer:
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use ceres's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.ceres.net/") as resp:
                    if resp.status == 200:
