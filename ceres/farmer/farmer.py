...
import ceres.server.ws_connection as ws  # lgtm [py/import-and-import-from]
from ceres.consensus.coinbase import create_puzzlehash_for_pk
from ceres.consensus.constants import ConsensusConstants
from ceres.daemon.keychain_proxy import (
    KeychainProxy,
    KeychainProxyConnectionFailure,
    connect_to_keychain_and_validate,
    wrap_local_keychain,
)
from ceres.pools.pool_config import PoolWalletConfig, load_pool_config
from ceres.protocols import farmer_protocol, harvester_protocol
from ceres.protocols.pool_protocol import (
    ErrorResponse,
...
from ceres.server.outbound_message import NodeType, make_msg
from ceres.server.server import ssl_context_for_root
from ceres.server.ws_connection import WSChiaConnection
from ceres.ssl.create_ssl import get_mozilla_ca_crt
...
from ceres.util.bech32m import decode_puzzle_hash
from ceres.util.byte_types import hexstr_to_bytes
from ceres.util.config import load_config, save_config, config_path_for_filename
...
from ceres.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self):
        ...
        if len(self.pool_sks_map) == 0:
            error_str = "No keys exist. Please run 'ceres keys generate' or open the UI."
            raise RuntimeError(error_str)
